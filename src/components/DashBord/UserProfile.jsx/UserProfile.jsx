...
const UserProfile = () => {
  ...
  console.log(userSingleData,"Loading")

  if(isloading){
    return <Loader></Loader>
  }
    return (
        <div className="w-full border min-h-screen bg-red p-5">


			
			

            <h1 className="text-[30px] uppercase">WelCome, <span className="text-[45px] text-yellow-800 font-[700]">{userSingleData?.name}</span></h1>

            {/*  */}

            <div className="flex flex-col w-full md:w-[50%] lg:w-[50%] justify-center  p-6 shadow-xl rounded-xl  bg-white mt-4 m-auto dark:text-gray-800">
	<img src={userSingleData?.image} alt="" className="w-60 h-60 mx-auto rounded-full bg-white aspect-square" />
	<div className="space-y-4 text-center divide-y dark:divide-gray-300">
		<div className="my-2 space-y-1">
			<h2 className="text-xl font-semibold sm:text-2xl uppercase">{userSingleData?.name}</h2>
			<p className="py-5 border text-xs sm:text-base dark:text-gray-600 uppercase">{userSingleData?.role}</p>
			<p className="py-5 border text-xs sm:text-base dark:text-gray-600 ">{userSingleData?.email}</p>
		</div>
